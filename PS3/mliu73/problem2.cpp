...
void setString(string &address2, string address3)//当写成setString(string address2, string address3)的时候，会创建一个新的copy，然后被改动的是那个copy而不是这个集合。所以要写成setString(类型 &名称1［也就是address-of operator］， 类型 名称2)，这样才能改变本身的数值。
{
    address2 = address3;
    return;
}
