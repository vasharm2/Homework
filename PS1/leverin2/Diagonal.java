...
public class Diagonal {
	...
	public static int diagonal(int[][] input) {
		...
		for (int counter = 0; counter < input.length; counter++) {
			diagonal[counter] = input[counter][counter];
		}
		int sum = 0;
		...
		int[] otherDiagonal = new int[input.length];
		for (int counter = 0; counter < input.length; counter++){
			otherDiagonal[counter] = input[counter][input.length - 1 - counter];
		}
		int otherSum = 0;
		for (int i = 0; i < input.length; i++) {
			otherSum += otherDiagonal[i];
		}
